...
try:
    Query = '''create table {}(Hospital_ID int(6),
               Hospital_name varchar(100),
               Password varchar(100))'''.format("hospital_list")
    cur.execute(Query)
    con.commit()
except Exception:
    pass
...
def check(Id):
    ...
    if Id in l:
        ...
        print('''You havent registered with us.
              \nPlease Register a Hospital and try again.''')
        time.sleep(1)
...
def register():
    ...
    query = '''create table {} (Full_Name varchar(50),
            Age int(3),
            Blood_Group varchar(4),
            Sex varchar(15),
            Contact_Number bigint,
            Address varchar(200),
            Amount_of_blood_required_in_mL float(6),
            Date_of_request date default(curdate()))'''.format("h_" + str(x))
    cur.execute(query)
    con.commit()
...
def Eject(Id):
    ...
    query1 = '''delete from {} where
                Amount_of_blood_required_in_mL={}'''.format("h_" + str(Id),
                                                              magic_no)
    query2 = '''delete from {} where
                Amount_of_blood_donated_in_mL={}'''.format("bloodbank",
                                                            magic_no)
    cur.execute(query1)
    con.commit()
    cur.execute(query2)
                    ...
                    for donors in rbank:
                        if donors[0] == d_name:
                            if donors[6] < req_bld_amt:
                                ...
                                query = '''update {}
                                           set
                                           Amount_of_blood_donated_in_mL
                                           = {} where
                                           Full_Name="{}"'''.format("bloodbank",
                                                                    rem_don_bld_amt,
                                                                    d_name)
                                query2 = '''update {}
                                           set
                                           Amount_of_blood_required_in_mL
                                           = {} where
                                           Full_Name="{}"'''.format("h_" + str(Id),
                                                                    rem_rec_bld_amt,
                                                                    n)
                                time.sleep(1)
                                print(f'''{rem_rec_bld_amt} is still remaining to be given.
                                      \nPlease give the remaining blood.''')
                                cur.execute(query)
                                con.commit()
                                cur.execute(query2)
                                con.commit()
                                ...
                                query = '''update {} set
                                           Amount_of_blood_donated_in_mL = {}
                                           where Full_Name="{}"'''.format("bloodbank",
                                                                           rem_don_bld_amt,
                                                                           d_name)
                                query2 = '''update {} set
                                            Amount_of_blood_required_in_mL = {}
                                            where Full_Name="{}"'''.format("h_" + str(Id),
                                                                           rem_rec_bld_amt,
                                                                           n)
                                cur.execute(query)
                                con.commit()
