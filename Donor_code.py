...
try:
    Query ="create table {}(Full_Name varchar(50),Age int(3),Blood_Group varchar(4),Sex varchar(15),Contact_Number bigint,Address varchar(200),Amount_of_blood_donated_in_mL float(6),Date_of_donation date default(curdate()))".format('bloodbank')
    cur.execute(Query)
    con.commit()
except Exception:
    pass
...
def show_don_list():
    ...
    for j in s:
        d = dict(zip(["Full Name", "Age","Blood Group", "Sex", "Contact Number", "Address", "Amount of blood donated in mL","Date_of_donation"], j))
        time.sleep(1)
